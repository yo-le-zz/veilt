...
### Added
- `pip install veilt` — installable as a library (`import veilt`) and a
  CLI (`veilt`), with prebuilt wheels for Windows x64, Linux x86_64, and
  Linux ARM64 (Raspberry Pi 5).
- Native secure-memory engine rewritten in pybind11 (`veilt._veilt_native`),
...
- Pure-Python fallback engine (`veilt.memory`) used automatically if no C++
...
- Tamper-evident, hash-chained audit log (`veilt.logs`), inspired by
...
- Optional OS-native secret store integration (`veilt.osvault`): zero-dependency
...
- FR/EN CLI and log messages (`veilt.i18n`, `--lang fr|en`).
- Cross-platform, XDG/AppData-compliant data directories (`veilt.config`) -
...
- `derive_master_key(password, "veilt_salt")` used a hardcoded literal salt
